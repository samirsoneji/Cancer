...
create.datos.fxn <- function(mx, prop, year.list) {
  ...
  for (y in 1:length(year.list)) {
    ...
    ex.localized <- lifetab.grad.nax.nLxappr(nMx=mx.localized,nax=nax, Rdx=1)$ex[10]
    ex.regional <- lifetab.grad.nax.nLxappr(nMx=mx.regional,nax=nax, Rdx=1)$ex[10]
    ex.distant <- lifetab.grad.nax.nLxappr(nMx=mx.distant,nax=nax, Rdx=1)$ex[10]
...
create.datos.prostate.fxn <- function(mx, prop, year.list) {
  ...
  for (y in 1:length(year.list)) {
    ...
    mx.distant[mx.distant==Inf] <- 10^6 #assigned infinite mortality rate a large numeric mortality rate
    ex.localized.regional <- lifetab.grad.nax.nLxappr(nMx=mx.localized.regional,nax=nax, Rdx=1)$ex[10]
    ex.distant <- lifetab.grad.nax.nLxappr(nMx=mx.distant,nax=nax, Rdx=1)$ex[10]
...
create.datos.lymphoma.fxn <- function(mx, prop, year.list) {
  ...
  for (y in 1:length(year.list)) {
    ...
    ex.stageI <- lifetab.grad.nax.nLxappr(nMx=mx.stageI,nax=nax, Rdx=1)$ex[10]
    ex.stageII <- lifetab.grad.nax.nLxappr(nMx=mx.stageII,nax=nax, Rdx=1)$ex[10]
    ex.stageIII <- lifetab.grad.nax.nLxappr(nMx=mx.stageIII,nax=nax, Rdx=1)$ex[10]
    ex.stageIV <- lifetab.grad.nax.nLxappr(nMx=mx.stageIV,nax=nax, Rdx=1)$ex[10]
