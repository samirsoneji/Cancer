library(plyr)
load("~/Desktop/Cancer/data/mx75.Rdata")
...
draws.fxn <- function(year.select,N,mx,ages) {
    ...
    spline.all <- spline(lt.all$x,lt.all$lx/lt.all$lx[1], xmin=min(lt.all$x), xmax=max(lt.all$x), n=101, method="fmm")
    ...
    draws.all <- sample(spline.all$x[-length(spline.all$x)]+ax[-length(ax)],size=N,replace=TRUE,prob=prob.all)
   
    spline.bc <- spline(lt.bc$x,lt.bc$lx/lt.bc$lx[1], xmin=min(lt.bc$x), xmax=max(lt.bc$x), n=101, method="fmm")
    spline.bc$y[which(spline.bc$y<0)] <- 0; spline.bc$y[which(spline.bc$y>=1)] <- 1
    prob.bc <- -1*diff(spline.bc$y)
    prob.bc[which(prob.bc<0)] <- 0
    draws.bc <- sample(spline.bc$x[-length(spline.bc$x)]+ax[-length(ax)],size=N,replace=TRUE,prob=prob.bc)

    spline.other <- spline(lt.other$x,lt.other$lx/lt.other$lx[1], xmin=min(lt.other$x), xmax=max(lt.other$x), n=101, method="fmm")
    ...
    draws.other <- sample(spline.other$x[-length(spline.other$x)]+ax[-length(ax)],size=N,replace=TRUE,prob=prob.other)
  
    return(list(draws.all=draws.all,draws.bc=draws.bc,draws.other=draws.other,lt.all=lt.all,lt.bc=lt.bc,lt.other=lt.other))
}

ages <- 0:100
tmp.1975 <- draws.fxn(1975,10,mx75,ages)
