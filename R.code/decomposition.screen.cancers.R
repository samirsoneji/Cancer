...
prostate <- results.fxn(mx.prostate, mx.prostate.cause, prop.prostate, "prostate", c(year0,year1))
...
polygon(c(year,rev(year)),
        ...
        col=color[4],border=FALSE)
axis(1,at=c(1973:2001))
axis(2,at=seq(0,100,10),las=1)
text(1990,100*(prop.prostate["1990",1]/2),"In Situ",cex=0.66)
text(1987,100*(sum(prop.prostate["1997",1])+ prop.prostate["1997",2]/2),"Localized-Regional",cex=0.66)
text(1987,100*(sum(prop.prostate["1997",1:2])+ prop.prostate["1997",3]/2),"Distant",cex=0.66,col="white")

stand.mx <- apply(mx.prostate.cause,c(1,3,4),function(x) sum(x * stand.male, na.rm=TRUE))
stand.mx <- stand.mx[,,c(3,2)]
matplot(year,log10(stand.mx["prostate",as.character(year),]),col=color[3:4],lty=1,bty="l",xlab=NA,ylab=NA,axes=FALSE,type="l")
matlines(year,log10(stand.mx["other",as.character(year),]),col=color[3:4],lty=2)
axis(1,at=c(1973:2001))
axis(2,at=seq(-5,0,0.5),paste(round(100*(10^seq(-5,0,0.5)),1)),las=1)
legend("bottomleft",ncol=1,cex=0.5,paste(c("Distant, Cancer","Distant, Other","Localized-Regional, Cancer","Localized-Regional, Other")),lty=1:2,col=c(color[4],color[4],color[3],color[3]),
       text.col=c(color[4],color[4],color[3],color[3]),bty="n")
...
text(xpos[2],sum(unlist(summary.prostate[3]))+summary.prostate[[4]]/2,"Localized-Regional,\nCancer",cex=0.66)
