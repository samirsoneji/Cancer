create.datos.fxn <- function(mx, prop, year.list) {
  value.list <- c("ex.overall","ex.insitu","ex.localized","ex.regional","ex.distant","prop.insitu","prop.localized","prop.regional","prop.distant")
  ...
  for (y in 1:length(year.list)) {
    mx.insitu <- c(rep(0,9),mx[,as.character(year.list[y]),"0. in situ"])
    ...
    ex.insitu <- lifetab.grad.nax.nLxappr(nMx=mx.insitu, Rdx=1)$ex[1]
    ex.localized <- lifetab.grad.nax.nLxappr(nMx=mx.localized, Rdx=1)$ex[1]
    ex.regional <- lifetab.grad.nax.nLxappr(nMx=mx.regional, Rdx=1)$ex[1]
    ex.distant <- lifetab.grad.nax.nLxappr(nMx=mx.distant, Rdx=1)$ex[1]
    prop.insitu <- prop[as.character(year.list[y]),"0. in situ"]
    ...
    ex.overall <- ex.insitu*prop.insitu+ex.localized*prop.localized+ex.regional*prop.regional+ex.distant*prop.distant
    datos[y,"ex.overall"] <- ex.overall
    datos[y,"ex.insitu"] <- ex.insitu
    datos[y,"ex.localized"] <- ex.localized
    datos[y,"ex.regional"] <- ex.regional
    datos[y,"ex.distant"] <- ex.distant
    datos[y,"prop.insitu"] <- prop.insitu
...
create.datos.prostate.fxn <- function(mx, prop, year.list) {
  value.list <- c("ex.overall","ex.insitu","ex.localized.regional","ex.distant","prop.insitu","prop.localized.regional","prop.distant")
  datos <- matrix(NA, nrow=length(year.list), ncol=length(value.list))
  rownames(datos) <- year.list
  colnames(datos) <- value.list
  for (y in 1:length(year.list)) {
    ...
    mx.distant[mx.distant==Inf] <- 10^6 #assigned infinite mortality rate a large numeric mortality rate
    ex.insitu <- lifetab.grad.nax.nLxappr(nMx=mx.insitu, Rdx=1)$ex[1]
    ex.localized.regional <- lifetab.grad.nax.nLxappr(nMx=mx.localized.regional, Rdx=1)$ex[1]
    ex.distant <- lifetab.grad.nax.nLxappr(nMx=mx.distant, Rdx=1)$ex[1]
    prop.insitu <- prop[as.character(year.list[y]),"0. in situ"]
    prop.localized.regional <- prop[as.character(year.list[y]),"8. localized.regional"]
    prop.distant <- prop[as.character(year.list[y]),"4. distant"]
    ex.overall <- ex.insitu*prop.insitu+ex.localized.regional*prop.localized.regional+ex.distant*prop.distant
    datos[y,"ex.overall"] <- ex.overall
    datos[y,"ex.insitu"] <- ex.insitu
...
create.datos.lymphoma.fxn <- function(mx, prop, year.list) {
  ...
  for (y in 1:length(year.list)) {
    ...
    ex.stageI <- lifetab.grad.nax.nLxappr(nMx=mx.stageI, Rdx=1)$ex[1]
    ex.stageII <- lifetab.grad.nax.nLxappr(nMx=mx.stageII, Rdx=1)$ex[1]
    ex.stageIII <- lifetab.grad.nax.nLxappr(nMx=mx.stageIII, Rdx=1)$ex[1]
    ex.stageIV <- lifetab.grad.nax.nLxappr(nMx=mx.stageIV, Rdx=1)$ex[1]
